...
public class ResourceController : MonoBehaviour
{
    ...
    private int _level
    {
        set
        {
            // untuk menyimpan value yang di set ke _level pada progress data
            UserDataManager.Progress.ResourcesLevels[_index] = value;
            UserDataManager.Save();
        }
        get
        {
            // untuk mengecek apakah index sudah terdapat pada progress data
            if(!UserDataManager.HasResources(_index))
            {
                // untuk jika tidak maka tampilkan level 1
                return 1;
            }
            // untuk jika iya maka tampilkan berdasarkan Progress data
    ...
    public void SetUnlocked (bool unlocked)
    {
        ...
        if(unlocked)
        {
            // untuk jika resources baru di unlock dan belum ada di progress data, maka tambahkan data baru
            if(!UserDataManager.HasResources(_index))
            {
